...
$filtered = array_values(array_filter($tasks, function($t) use ($fFrom, $fTo, $fEmployee) {
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
  ...
  <div class="d-flex gap-2">
    <button class="btn btn-outline-success"
            onclick="exportTableExcel('reportTable','TBI_Report_<?= $reportType ?>')">
      <i class="bi bi-file-earmark-excel me-1"></i>Excel
    </button>
    <button class="btn btn-outline-danger"
            onclick="exportTablePDF('reportTable','<?= e($periodLabels[$reportType] ?? 'Report') ?>')">
  ...
  <div class="card-body">
    <div class="d-flex flex-wrap gap-2 mb-3">
      ...
      $types = ['daily'=>'Daily','weekly'=>'Weekly','monthly'=>'Monthly',
                'custom'=>'Custom Range','pending'=>'Pending Tasks','employee'=>'Employee Productivity'];
  ...
  <div class="card-body">
    ...
    <div class="row g-3 mb-4">
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-primary"><?= $rTotal ?></div>
        <div class="small text-muted">Total Tasks</div>
      </div>
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-success"><?= $rApproved ?></div>
        <div class="small text-muted">Approved</div>
      </div>
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-info"><?= $rCompleted ?></div>
        <div class="small text-muted">Completed</div>
      </div>
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-warning"><?= $rPending ?></div>
        <div class="small text-muted">Pending</div>
      </div>
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-danger"><?= $rOverdue ?></div>
        <div class="small text-muted">Overdue</div>
      </div>
      <div class="col-6 col-md-2 text-center">
        <div class="fw-700 fs-4 text-purple">
          <?= $rTotal > 0 ? round(($rApproved/$rTotal)*100) : 0 ?>%
        </div>
      ...
      <table class="table table-bordered table-sm" id="reportTable">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="<?= $empStats['overdue'] > 0 ? 'text-danger fw-600' : '' ?>">
              <?= $empStats['overdue'] ?>
            </td>
    ...
    <div class="table-responsive">
      <table class="table table-bordered table-sm"
             id="<?= $reportType !== 'employee' ? 'reportTable' : 'detailTable' ?>">
        <thead class="table-primary">
          <tr>
            <th>#</th><th>Task</th><th>Employee</th><th>Priority</th>
            <th>Assigned</th><th>Deadline</th><th>Days Overdue</th><th>Status</th><th>Assigned By</th>
          </tr>
        </thead>
        <tbody>
