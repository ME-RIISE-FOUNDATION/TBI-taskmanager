...
// Sort employees by designation — strict !== false check (fixes usort bug)
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
  <div>
    <h4 class="fw-700 mb-0">
      <i class="bi bi-grid-fill me-2 text-primary"></i>Admin Dashboard
    </h4>
    <small class="text-muted">
      Welcome back, <?= e($_SESSION['name']) ?> &bull; <?= date('l, d F Y') ?>
    </small>
  </div>
  ...
  <div class="col-6 col-sm-4 col-lg-2">
    ...
    <?php endif; ?>
      <div class="stat-card border-start border-4" style="border-left-color:var(--c-purple)!important">
        ...
        <div class="stat-lbl">
          Pending Approvals
          <?= $pendingApprovals > 0 ? '<span class="badge bg-danger ms-1">!</span>' : '' ?>
        </div>
      </div>
...
<h5 class="fw-700 mb-3">
  <i class="bi bi-people-fill me-2 text-primary"></i>Employee Overview
</h5>

<div class="row g-3 mb-4" id="employeeCards">
<?php foreach ($employees as $emp):
  $stats    = employeeStats($tasks, $emp['Employee_ID'] ?? '');
  $nameWords= explode(' ', $emp['Name'] ?? 'U');
  $initials = substr(implode('', array_map(fn($w) => strtoupper($w[0]), $nameWords)), 0, 2);
...
<div class="col-12 col-sm-6 col-lg-4">
  <a href="<?= BASE_URL ?>/admin/tasks.php?employee=<?= urlencode($emp['Employee_ID'] ?? '') ?>"
     class="emp-card text-decoration-none">

    <div class="emp-card-header">
      <?php if (!empty($emp['Photo_URL'])): ?>
        <img src="<?= e($emp['Photo_URL']) ?>" alt="<?= e($emp['Name'] ?? '') ?>"
             class="emp-avatar">
      <?php else: ?>
    ...
    <div class="emp-card-footer">
      <i class="bi bi-envelope me-1"></i><?= e($emp['Email'] ?? '—') ?>
    </div>
    ...
    <div class="table-responsive">
      <table class="table table-hover mb-0">
        <thead class="table-light">
          <tr>
            <th>Task</th><th>Employee</th><th>Priority</th>
          ...
          <tr class="<?= $overdue ? 'table-danger' : '' ?>">
            <td>
              ...
              <small class="text-danger">
                <i class="bi bi-exclamation-circle me-1"></i>
                Overdue <?= daysPending($t['Deadline'] ?? '') ?>d
              </small>
              <?php endif; ?>
            </td>
...
(function(){
  ...
  const taskData= <?= json_encode(array_map(fn($e) => employeeStats($tasks, $e['Employee_ID'] ?? ''), $employees)) ?>;
  new Chart(ctx, {
    type:'bar',
    data:{
      labels: labels,
      datasets:[
        { label:'Total',     data: taskData.map(d=>d.total),     backgroundColor:'rgba(96,165,250,.55)' },
        { label:'Completed', data: taskData.map(d=>d.completed), backgroundColor:'rgba(74,222,128,.55)' },
        { label:'Pending',   data: taskData.map(d=>d.pending),   backgroundColor:'rgba(250,204,21,.55)' },
      ]
    },
    options:{
      responsive:true, maintainAspectRatio:false,
